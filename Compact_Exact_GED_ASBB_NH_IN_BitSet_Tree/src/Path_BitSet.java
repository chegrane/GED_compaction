...
public class Path_BitSet extends Path_Node{
    ...
    public Path_BitSet(Path_Node path_node)
    {
        ...
        // this in th case of dynamic version
        if(path_node instanceof Path_Tree)
        {
            List<Integer> old_Edit_Operation_List_ids = path_node.get_Operations_ids_List();

            for(Integer id: old_Edit_Operation_List_ids)
            {
                this.list_vertex_edit_operations.set(id);
            }
        }
        else
        {
    ...
    protected void compute_h_and_newUpperBound()
    {
        ...
        if(n1>0 && n2>0)
        {
            ...
            for (int i=0; i<assignment.length; i++)
            {
                ...
                child_path = new Path_BitSet(father_path);
