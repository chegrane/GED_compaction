...
public class Compact_Exact_GED_AStar_BitSet_Tree {
     ...
     * Default Constructor
     */
    public Compact_Exact_GED_AStar_BitSet_Tree(Graph g1, Graph g2)
    {
    ...
    public Compact_Exact_GED_AStar_BitSet_Tree(Graph g1, Graph g2, long amount_RunTime) {
        comparator = new Path_Cost_Comparator();
        OPEN = new PriorityQueue<Path_Node>(10, comparator);
    ...
    public double computeGED() {
        ...
        {// ------------------- generate the first level of the search Tree :
            ...
            for (int j = 0; j < Graph_Map.nb_vertices_g2; j++)
            {
                ...
                if(compute_Used_Space)
                {
                    size_all_data_used+=meter.measureDeep(path_node);
                }
            }
            ...
            OPEN.add(path_node_delete);
            nb_all_path_added_to_open++;

            if(compute_Used_Space)
            {
                size_all_data_used+=meter.measureDeep(path_node_delete);
            }
        }
