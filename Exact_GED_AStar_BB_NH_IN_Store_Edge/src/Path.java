...
public class Path {
    ...
    private void compute_h_and_newUpperBound()
    {
        ...
        if(n1>0 && n2>0)
        {
            ...
            for (int i=0; i<assignment.length; i++)
            {
                ...
                {
                    Edit_Operation vertex_substitution_operation = new Edit_Operation(Graph_Map.list_vertices_g1.get(this.index_processed_vertices_g1 + i1), remaining_unprocessed_vertex_g2.get(j1));
                    child_path.add_to_real_path(vertex_substitution_operation);
                }
                else if(i1<n1 && (j1>=n2 && j1<n2+n1)) // Upper right == DELETION : delete the vertex i1 from g1
                {
                    Edit_Operation vertex_deletion_operation = new Edit_Operation(Graph_Map.list_vertices_g1.get(this.index_processed_vertices_g1 + i1), null);
                    child_path.add_to_real_path(vertex_deletion_operation);
                }
                else if((i1>=n1 && i1<n1+n2) && j1<n2) // Bottom left == insertion : insert the vertex j1 in g2
                {
                    Edit_Operation newOp3 = new Edit_Operation(null, remaining_unprocessed_vertex_g2.get(j1));
                    child_path.add_to_real_path(newOp3);
                }
        ...
        else // at this level, either n <=0  or m <=0
        {
            ...
            if(n1>0) // g1: n1 , it's mean we delete all n vertex of g1
            {
                for(int i=this.index_processed_vertices_g1;i<Graph_Map.nb_vertices_g1;i++)
                {
                    ...
                    Edit_Operation vertex_deletion_operation = new Edit_Operation(Graph_Map.list_vertices_g1.get(i), null);
                    child_path.add_to_real_path(vertex_deletion_operation);
            ...
            {
                for(Vertex v:remaining_unprocessed_vertex_g2)
                {
                    child_path = new Path(father_path);

                    Edit_Operation newOp3 = new Edit_Operation(null, v);
                    child_path.add_to_real_path(newOp3);
     ...
     */
    @SuppressWarnings("unchecked")
    public boolean add_to_real_path(Edit_Operation edit_operation)
    {
        boolean source_Edge_Exists;
        boolean target_Edge_Exists;
